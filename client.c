...
unsigned char verify_cert_list(struct cert_list_st *list, struct cert_list_st *trusted);
...
int main(int argc, char *argv[])
{
  ...
  for (i=0; i<NUM_OF_PROBLEMS; i++)
  {
    ...
    validation = verify_cert_list(list, trusted);
...
unsigned char verify_cert_list(struct cert_list_st *list, struct cert_list_st *trusted)
{
  ...
  struct cert_entry_st *entry;
  struct cert_entry_st *issuer_cert;
  struct keypair *pubkey;
  int slen;
  unsigned char *sign;
  int mlen;
  unsigned char *msg;

  entry = get_cert_entry_from_list(0, list);

  do
  {
  issuer_cert = get_issuer_cert_entry_from_list(entry, list);
  if (issuer_cert == NULL){ return NO_ISSUER_FOUND; }

  pubkey = get_pubkey_from_certificate(issuer_cert);
  sign = get_signature_from_certificate(entry, &slen);
  msg = get_message_from_certificate(entry, &slen);

  entry = issuer_cert

  if (rsa_verify_message(pubkey, sign, slen, msg, mlen) != SUCCESS){ return WRONG_SIGNATURE; }
  }while(is_self_signed_certificate(issuer_cert) == FAILURE);

  if(is_trusted_root_certificate(issuer_cert, trusted)){ return CHAIN_VALIDATED; }
  else { return NO_TRUSTED_ROOT; }
...
int is_trusted_root_certificate(struct cert_entry_st *entry, struct cert_list_st *trusted)
{
  ...
  char num;
  struct cert_entry_st *root;

  num = get_num_of_entry_in_list(trusted);
  for(int i=0; i<num; i++){
    root = get_cert_entry_from_list(i, trusted);
    if(is_name_same(get_subject_from_certificate(root),get_subject_from_certificate(entry))==SUCCESS){
        if(is_name_same(get_issuer_from_certificate(root),get_issuer_from_certificate(entry))==SUCCESS){
            return 1;
        }
    }
